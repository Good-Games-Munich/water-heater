...
export class PayoutService {
    public calculatePayout(amountToPayout: number): {
        ...
        const fiftyPercent = Math.round(amountToPayout * 100) / 100;
        const thirtyPercent = Math.round(amountToPayout * 100) / 100;
        const twentyPercent = Math.round(amountToPayout * 100) / 100;
